...
class VyvihiViewController: UIViewController, UICollisionBehaviorDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
    
    @IBOutlet weak var collectionView: UICollectionView!
    @IBOutlet weak var pageControl: UIPageControl!
    
    
    let data = ["Признаки вывиха ↑", "Оказание первой помощи",]
    let textViewData = [
        "Вывихнутый сустав временно деформирован и обездвижен\nВозникает сильная боль в области вывихнутого сустава\nВозникает отек в области вывиха",
        "Оказание первой медицинской помощи при вывихах:\n1. Обеспечить поврежденной конечности покой\n2. Наложить на поврежденное место тугую повязку\n3. Дать пострадавшему обезболивающее средство\n4. Доставить пострадавшего в мед.учреждение",
                        ]
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(VyvihiViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! VyvihiCollectionViewCell
