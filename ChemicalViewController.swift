...
class ChemicalViewController: UIViewController, UICollisionBehaviorDelegate {
    
    @IBOutlet weak var collectionView: UICollectionView!
    @IBOutlet weak var pageControl: UIPageControl!
    let data = ["При ожогах щелочью ↑", "При ожогах кислотой","При ожогах известью", "При ожогах фосфором"]
    let textViewData = ["На место ожога наложить кислоту",
                        "На место ожога наложить щелочь",
                        "При ожогах негашёной известью, на место ожога нанести масло. /Ни в коем случае не допускайте контакт с водой", "При ожогах фосфором, место ожога нейтрализовать водой"
                        ]
    ...
    let imageArray = ["ozh1","ozh2","ozh3","ozh4","ozh5"]
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(ChemicalViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! ChamicalCollectionViewCell
