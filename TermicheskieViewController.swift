//
//  TermicheskieViewController.swift
//  FirstAidNewVersion
//
...
class TermicheskieViewController: UIViewController, UICollisionBehaviorDelegate, UICollectionViewDataSource, UICollectionViewDelegate {

    let data = ["Шаг 1 ↑", "Шаг 2","Шаг 3", "Категорический запрещается!!!"]
    let textViewData = ["Облить струей холодной воды место ожога в течение 5-10 минут.",
                        "Накрыть сухой чистой тканью.",
                        "Поверх ткани приложить холод", "1) Намазывать место ожога кремами, жирами, мукой или крахмалом\n2) Вскрывать пузыри, удалять жидкость"
                        ]
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(TermicheskieViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! TermicheskieCollectionViewCell
