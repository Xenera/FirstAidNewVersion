...
class DyhanieViewController: UIViewController, UICollisionBehaviorDelegate {
    ...
    let data = ["Искусственное дыхание"]
    let textViewData = [
    "    1. Правой рукой обхватить подбородок так, чтобы пальцы, расположенные на нижней челюсти и щеках пострадвашего смогли разжать и приоткрыть губы.\n2. Левой рукой зажать нос\n3. Запрокинуть голову пострадавшего. Удерживать голову пострадавшего в таком положении до оканчания проведения вдоха.\n4. Плотно прижаться губами к губам пострадавшего и сделать выдох. Если во время вдоха спасатель почувствует рауздвание щек, то он может сдалеть вывод о неэффективности попытки вдоха"
    ]
    var views = [UIView]()
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(DyhanieViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! DyhanieCollectionViewCell
