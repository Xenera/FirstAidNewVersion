...
class VenoznoeViewController: UIViewController, UICollisionBehaviorDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
    
    
    @IBOutlet weak var pageControl: UIPageControl!
    @IBOutlet weak var collectionView: UICollectionView!
    
    let imageArray = ["венозное.png","ven1.png","ven2.png","ven3.png"]
    let data = ["Шаг 1 ↑", "Шаг 2","Шаг 3"]
    let textViewData = [
        "Во время венозного кровотечении кровь стекает медленно и без резких выбросов. Цвет крови приобретает темно-вишневый оттенок.",
        "1) Наложить давящую повязку",
        "2) Доставить пострадавшего в мед.учреждение или вызвать скорую помощь"
    ]
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(VenoznoeViewController.handlePan(_:)))
