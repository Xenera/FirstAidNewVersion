...
class OtkrytyeViewController: UIViewController, UICollisionBehaviorDelegate, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    let textViewData = [
        ...
        "Оказание первой медицинской помощи при открытых переломах:\n1. Остановить кровотечение и обработать края раны антисептиком\n2. На рану в области перелома наложить стерильную повязку\n3. Дать пострадавшему обезболивающее средство\n4. Провести иммобилизацию(обездвиживание) конечности в том положении, в котором она оказалась в момент повреждения\n5. Доставить пострадавшего в мед.учреждение",
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(OtkrytyeViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! OtkrytyeCollectionViewCell
