...
class VnutrViewController: UIViewController, UICollisionBehaviorDelegate, UICollectionViewDataSource, UICollectionViewDelegate {

    @IBOutlet weak var collectionView: UICollectionView!
    @IBOutlet weak var pageControl: UIPageControl!
    let data = ["Шаг 1 ↑", "Шаг 2","Шаг 3"]
    let textViewData = [
        "При артериальном кровотечении кровь бьет из раны фонтаном.Остановка артериального кровотечения: - прижать пальцами или кулаком артерию; - точка прижатия должна быть выше места кровотечения - до наложения жгута держать конечность в приподнятом положении.",
    ...
    func addVC(atOffset offset:CGFloat, textForHeader data: AnyObject?, textForTextView text: AnyObject?) -> UIView? {
        ...
        if let view = stackElementVC.view {
            ...
            let panGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(VnutrViewController.handlePan(_:)))
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCellWithReuseIdentifier("cell", forIndexPath: indexPath) as! VnutrCollectionViewCell
